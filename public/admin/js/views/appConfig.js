...
async function fetchAndRenderConfig(container) {
  try {
    ...
    const booleanFlags = settings.filter((s) => s.valueType === 'boolean' || s.value === 'true' || s.value === 'false');
    const textSettings = settings.filter((s) => s.valueType !== 'boolean' && s.value !== 'true' && s.value !== 'false');
